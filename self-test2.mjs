...
// 模拟一个页面的OCR结果（先按设备空间写，再转成用户空间）
// 第一行：valid until 01 June 2031 (设备空间 y=100)
const line1Dev = [
...
// 第二行：Completion date ... 02 June 2026 (设备空间 y=130)
const line2Dev = [
...
// 第三行：Issued at ... 02 June 2026 (设备空间 y=160)
const line3Dev = [
...
// 转换成用户空间（unified格式：y是顶部y=大的值，height正数）
function toUnified(devItems: typeof line1Dev) {
  return devItems.map(d => ({
    str: d.str,
    x0: d.x,
    y: toUserY(d.y),  // 设备空间顶部y → 用户空间顶部y（值大）
    width: d.w,
    height: d.h,
    page: 1,
  }));
...
if (expiry) {
  ...
  const expectedDevYTop = 100; // 设备空间顶部y
  const expectedDevYBot = 112; // 设备空间底部y
...
if (issue) {
  console.log(`ℹ️  签发日期: ${issue.date} (${issue.type})`);
}
